...
- **Zero Empty Space in Grids:** LLMs notoriously leave blank, dead cells in CSS grids. You MUST use Tailwind's `grid-flow-dense` (`grid-auto-flow: dense`) to ensure cells pack tight. Ensure all `col-span` and `row-span` values sum to matching rows/columns, leaving no voids or empty spots.
